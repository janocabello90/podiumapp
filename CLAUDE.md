...
### Fase G — Informe agregado de campaña (IA) (COMPLETADA 2026-07-29). Doc: `FASE-G-EQUIPOS.md`
- **Datos:** `reports.scope` ('individual'|'campaign', default 'individual') + `reports.campaign_id` NULLABLE (FK→campaigns ON DELETE SET NULL) + `reports.patient_id` **relajado a NULLABLE** + CHECK de coherencia (`individual`⇒patient_id; `campaign`⇒campaign_id). Migración `20260729085432_add_campaign_scope_to_reports`. Aditivo: informes existentes = `scope='individual'`. Se descartó `team_id` en reports (la campaña es la clave de scope).
- **Generación agregada:** `POST /api/reports/campaign-generate` (body `campaignId`): agrega las sesiones de la campaña en un **informe cualitativo v1** (resumen cualitativo por jugador: notas de valoración + notas por prueba + `tests.vald_interpretation_prompt` + `patients.vald_interpretation`), cap 40 jugadores, Claude Sonnet 4. `report_data` con secciones propias (portada, resumen, `hallazgos_por_equipo[]`, patrones/riesgos, fortalezas, `jugadores_a_vigilar[]`, recomendaciones, descargo) + `_meta` (cobertura, equipos). Guarda `scope='campaign'`, `patient_id=null`.
- **UI:** `/campaigns/[id]` → tarjeta "Informe de campaña" con `CampaignReportButton` (gating: ≥1 valorado; muestra cobertura X/N) + enlace al último. Revisión `/campaigns/[id]/report` (`CampaignReportView`): editar secciones · **aprobar** (PATCH `/api/reports/[id]`, genérico) · **PDF** (`POST /api/reports/export-pdf-campaign`, endpoint propio jsPDF, maquetación mínima v1).
