...
- **Generación de informes en segundo plano + traza de estado en la app (2026-08).** Hoy la generación es **síncrona**: el fisio tiene que dejar la pestaña abierta 1–3 min y no hay traza de progreso (solo el spinner; si falla, un toast; el detalle vive en los logs de Vercel). Propuesta: (a) escribir una fila `reports` con `status='generating'` al **inicio** (no solo al final), procesar por detrás (Vercel `waitUntil`/Fluid Compute o una cola), y pasar a `draft`/`error` al terminar; (b) que la lista/ficha muestre el estado (`generando…` / `borrador` / `error`) y se actualice sola (polling o refetch), para que se pueda lanzar y salir sin perder el resultado ni creer que ha fallado. Mejora la **trazabilidad** (estado + mensaje de error visibles en la propia app) y permite **disparar y olvidar**. Cambio de arquitectura pequeño-medio; hacer tras el arranque.

## 🧩 Estudios — editar equipos de un estudio existente (2026-08)
Hoy los equipos de un estudio (`campaign_teams`) **solo se fijan al crearlo** (`CreateStudyForm`/`CreateCampaignForm`); **no hay UI para añadir/quitar equipos después**. Consecuencia: si creas un equipo nuevo en el grupo (p. ej. "Cadiz Balonmano" en el grupo Cádiz) **después** de crear el estudio, no puedes incorporarlo a ese estudio. Workaround actual: crear un estudio nuevo. **Pendiente:** en `/estudios/[id]` añadir "Añadir/quitar equipos" (insert/delete en `campaign_teams`, admin). Ojo de diseño: mezclar deportes en un mismo estudio hace raro el **informe agregado** (mejor un estudio por deporte).
